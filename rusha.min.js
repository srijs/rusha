...
function Rusha(sizeHint){'use strict';var self$2={fill:0};var padlen=function(len){return len+1+(len%64<56?56:56+64)-len%64+8;};var padZeroes=function(bin,len){for(var i=len>>2;i<bin.length;i++)
bin[i]=0;};var padData=function(bin,len){bin[len>>2]|=128<<24-(len%4<<3);bin[((len>>2)+2&~15)+15]=len<<3;};var convStr=function(str,bin,len){var i;for(i=0;i<len;i=i+4|0){bin[i>>2]=str.charCodeAt(i)<<24|str.charCodeAt(i+1)<<16|str.charCodeAt(i+2)<<8|str.charCodeAt(i+3);}};var convBuf=function(buf,bin,len){var i,m=len%4,j=len-m;for(i=0;i<j;i=i+4|0){bin[i>>2]=buf[i]<<24|buf[i+1]<<16|buf[i+2]<<8|buf[i+3];}
...
return res.join('');};var ceilHeapSize=function(v){var p;if(v<=4096)
return 4096;if(v<16777216){for(p=1;p<v;p=p<<1);}else{for(p=16777216;p<v;p+=16777216);}
return p;};var resize=function(size){self$2.sizeHint=size;self$2.heap=new ArrayBuffer(ceilHeapSize(padlen(size)+320));self$2.core=RushaCore({Int32Array:Int32Array,DataView:DataView},{},self$2.heap);};resize(sizeHint||0);var coreCall=function(len){var h=new Int32Array(self$2.heap,len<<2,5);h[0]=1732584193;h[1]=-271733879;h[2]=-1732584194;h[3]=271733878;h[4]=-1009589776;self$2.core.hash(len);};var rawDigest=this.rawDigest=function(str){var len=str.byteLength||str.length;if(len>self$2.sizeHint){resize(len);}
var view=new Int32Array(self$2.heap,0,padlen(len)>>2);padZeroes(view,len);conv(str,view,len);padData(view,len);coreCall(view.length);var out=new Int32Array(5);var arr=new DataView(out.buffer);arr.setInt32(0,view[0],false);arr.setInt32(4,view[1],false);arr.setInt32(8,view[2],false);arr.setInt32(12,view[3],false);arr.setInt32(16,view[4],false);return out;};this.digest=this.digestFromString=this.digestFromBuffer=this.digestFromArrayBuffer=function(str){return hex(rawDigest(str).buffer);};};function RushaCore(stdlib,foreign,heap){'use asm';var H=new stdlib.Int32Array(heap);function hash(k){k=k|0;var i=0,j=0,y0=0,z0=0,y1=0,z1=0,y2=0,z2=0,y3=0,z3=0,y4=0,z4=0,t0=0,t1=0;k=k<<2;y0=H[k+0>>2]|0;y1=H[k+4>>2]|0;y2=H[k+8>>2]|0;y3=H[k+12>>2]|0;y4=H[k+16>>2]|0;for(i=0;(i|0)<(k|0);i=i+64|0){z0=y0;z1=y1;z2=y2;z3=y3;z4=y4;for(j=0;(j|0)<64;j=j+4|0){t1=H[i+j>>2]|0;t0=((y0<<5|y0>>>27)+(y1&y2|~y1&y3)|0)+((t1+y4|0)+1518500249|0)|0;y4=y3;y3=y2;y2=y1<<30|y1>>>2;y1=y0;y0=t0;;H[k+j>>2]=t1;}
